...
SELECT	@ProductName = 
		...
		@TheLastVersion = 
		CASE SUBSTRING(@ProductVersion,1,4)
		    WHEN '13.0' THEN 'SQL Server 2016 RTM' 
			WHEN '12.0' THEN 'SQL Server 2014 SP2' 
			WHEN '11.0' THEN 'SQL Server 2012 SP3 CU4' 
...
SELECT  @SPInfo = 
		CASE @ProductName
		        WHEN 'SQL Server 2016' THEN 
											CASE @ProductLevel
												WHEN 'RTM' THEN @Temp1 ELSE @Temp12 + 'RTM, <https://support.microsoft.com/en-us/kb/3177534>'
											END
				...
				WHEN 'SQL Server 2012' THEN
											CASE @ProductLevel
												WHEN 'SP3' THEN @Temp1 ELSE @Temp12 + 'SP3, <https://support.microsoft.com/en-us/kb/3072779>'
											END
		...
		CASE @ProductName
			    WHEN 'SQL Server 2016' THEN 
											CASE @ProductVersion
												WHEN '13.0.2149.0' THEN 'You have already installed the latest cumulative update.' 
												ELSE 'Install the latest Cumulative Update (CU) of RTM:  CU1, <https://support.microsoft.com/en-us/kb/3164674>'
											END
				WHEN 'SQL Server 2014' THEN 
											CASE @ProductVersion
												WHEN '12.0.5000.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP2:  RTW/PCU2, <https://support.microsoft.com/en-us/kb/2936603>'
											END
				WHEN 'SQL Server 2012' THEN
											CASE @ProductVersion
												WHEN '11.0.6540.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP3:  CU4, <https://support.microsoft.com/en-us/kb/3165264>'
											END
...
BEGIN
	...
	--2012
	IF (@ProductLevel = 'RTM') AND (@ProductVersion < '11.0.3000.00')
	BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP2') AND (@ProductVersion < '11.0.5655.0')
	BEGIN
		SET @CumulativeUpdate = 'CU13'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3165266';
	END
	ELSE IF (@ProductLevel = 'SP3') AND (@ProductVersion < '11.0.6540.0')
	BEGIN
		SET @CumulativeUpdate = 'CU4'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3165264';
	END
	SELECT  @OtherProduct = 
			CASE 
				WHEN @ProductLevel = 'RTM' THEN ' If you want to upgrade to higher edition, please intall service pack.'
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '12.0.4468.0')
	BEGIN
		SET @CumulativeUpdate = 'CU8'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3174038';
	END
	ELSE IF (@ProductLevel = 'SP2') AND (@ProductVersion < '12.0.5000.0')
	BEGIN
		SET @CumulativeUpdate = 'RTW/PCU2'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/2936603';
	END
...
BEGIN
	...
	BEGIN
		...
		PRINT 'Note, if you want to know information about CU, you need to intall' 
			   ...
			   + CHAR(13) + REPLICATE(' ',50) + '- see KB3094220 to get the Cumulative Update 10'	
	END
	...
	BEGIN
		...
		PRINT 'Note, if you want to know information about CU, you need to intall' 
			   ...
			   + CHAR(13) + REPLICATE(' ',50) + '- see KB3094221 to get the Cumulative Update 3'	
	END
...
IF (@ProductName = 'SQL server 2012')
BEGIN
	IF @ProductLevel = 'SP2' OR @ProductLevel = 'SP3'
		PRINT @MainSupport
	ELSE
